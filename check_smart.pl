...
# Apr 22, 2014: Jerome Lauret - implemented -g to do a global lookup
...
foreach $device ( split(":",$device) ){
    ...
    my $perf_string_local = "".join(' ', @perfdata);
	...
	if($exit_status ne 'OK'){
	    ...
	    $perf_string_local =~ s/\w+=0//g;
	    $perf_string      .= " [$tag] ".$perf_string_local;
	} else {
	    $status_string_local = $label." no SMART errors detected. ".join(', ', @error_messages);
	}
    } else {
...
sub print_help {
        print_revision($basename,$revision);
        print "\nUsage: $basename {-d=<block device>|-g=<block device family>} -i=(ata|scsi|3ware,N|areca,N|hpt,L/M/N|cciss,N|megaraid,N) [-b N] [--debug]\n\n";
	print "At least one of the below. -d supersedes -g\n";
        print "  -d/--device: a physical block device to be SMART monitored, eg /dev/sda\n";
	print "  -g/--global: a group of physical devices to be SMART monitored, eg /dev/sd\n";
	print "               will searrch for all sd devices and report errors globally.\n";
	print "\n";
	print "Other options\n";
